...
if origen_datos is not None:
    try:
        ...
        # ==============================================================================
        # DESPLIEGUE VISUAL (MÉTRICAS PRINCIPALES)
        # ==============================================================================
        ...
        col2.metric("📄 Volumen de Documentos Emitidos", f"{total_facturas_procesadas:,} Facturas")
        col3.metric("🧮 Ticket Promedio por Factura", f"$ {promedio_por_factura:,.0f}")

        st.markdown("---")

        # ==============================================================================
        # SECCIÓN 1: NUEVA ANALÍTICA DE COMPORTAMIENTO SEMANAL (RANGOS DE DÍAS)
        # ==============================================================================
        st.markdown("### ⏱️ Análisis del Comportamiento Semanal dentro del Mes")
        sem_col1, sem_col2 = st.columns(2)
        
        # Agrupar datos por rango semanal
        df_semanal_kpis = df_filtrado.groupby("RANGO_SEMANAL").agg(
            Monto_Total=('TOTAL', 'sum'),
            Cantidad_Total=('FOLIO', 'count')
        ).reset_index().sort_values(by="RANGO_SEMANAL")
        
        with sem_col1:
            st.subheader("Distribución del Pasivo por Rangos de Días")
            fig_sem_val = px.bar(
                df_semanal_kpis, x="RANGO_SEMANAL", y="Monto_Total",
                template="plotly_white", color_discrete_sequence=['#0D47A1'],
                labels={'RANGO_SEMANAL': 'Semana del Mes', 'Monto_Total': 'Valor Neto Total ($)'}
            )
            fig_sem_val.update_layout(yaxis_tickformat="$ ,.0f", separators=",.")
            fig_sem_val.update_traces(hovertemplate="<b>Rango:</b> %{x}<br><b>Pasivo Total:</b> $ %{y:,.0f}<extra></extra>")
            st.plotly_chart(fig_sem_val, use_container_width=True)
            
        with sem_col2:
            st.subheader("Carga Operativa: Volumen de Facturas por Rangos de Días")
            fig_sem_cant = px.bar(
                df_semanal_kpis, x="RANGO_SEMANAL", y="Cantidad_Total",
                template="plotly_white", color_discrete_sequence=['#1565C0'],
                labels={'RANGO_SEMANAL': 'Semana del Mes', 'Cantidad_Total': 'Cantidad de Facturas'}
            )
            fig_sem_cant.update_layout(yaxis_tickformat=",.0f", separators=",.")
            fig_sem_cant.update_traces(hovertemplate="<b>Rango:</b> %{x}<br><b>Cantidad:</b> %{y:,} Facturas<extra></extra>")
        ...
        with t_col1:
            ...
            df_dia_mes = df_filtrado.groupby("DIA_DEL_MES")["TOTAL"].sum().reset_index().sort_values(by="DIA_DEL_MES")
            fig_dia = px.bar(df_dia_mes, x="DIA_DEL_MES", y="TOTAL", template="plotly_white", color_discrete_sequence=['#1E88E5'], labels={'DIA_DEL_MES': 'Día del Calendario', 'TOTAL': 'Monto Facturado ($)'})
            fig_dia.update_layout(yaxis_tickformat="$ ,.0f", separators=",.")
            fig_dia.update_traces(hovertemplate="<b>Día:</b> %{x}<br><b>Monto Emitido:</b> $ %{y:,.0f}<extra></extra>")
            st.plotly_chart(fig_dia, use_container_width=True)
            
        with t_col2:
            st.subheader("Evolución de la Facturación Mensual 2026")
            df_mes = df_filtrado.groupby("MES_EMISION")["TOTAL"].sum().reset_index().sort_values(by="MES_EMISION")
            fig_mes = px.line(df_mes, x="MES_EMISION", y="TOTAL", markers=True, template="plotly_white", color_discrete_sequence=['#4CAF50'], labels={'MES_EMISION': 'Mes', 'TOTAL': 'Monto Facturado ($)'})
            fig_mes.update_layout(yaxis_tickformat="$ ,.0f", separators=",.", xaxis_title="Mes de Emisión")
            fig_mes.update_traces(hovertemplate="<b>Mes:</b> %{x}<br><b>Total Gasto:</b> $ %{y:,.0f}<extra></extra>")
        ...
        with g1:
            ...
            df_resumen_area = df_filtrado.groupby("RESPONSABLE")["TOTAL"].sum().reset_index().sort_values(by="TOTAL", ascending=False)
            fig_bar = px.bar(df_resumen_area, x='RESPONSABLE', y='TOTAL', template="plotly_white", color_discrete_sequence=['#4CAF50'], labels={'RESPONSABLE': 'Área', 'TOTAL': 'Monto ($)'})
            fig_bar.update_layout(yaxis_tickformat="$ ,.0f", separators=",.")
            fig_bar.update_traces(hovertemplate="<b>Área:</b> %{x}<br><b>Total Compras:</b> $ %{y:,.0f}<extra></extra>")
            st.plotly_chart(fig_bar, use_container_width=True)

        with g2:
            st.subheader("Top 10 Proveedores con Mayor Volumen de Facturación")
            df_prov_top = df_filtrado.groupby("NOMBRE EMISOR")["TOTAL"].sum().reset_index().sort_values(by="TOTAL", ascending=False).head(10)
            fig_prov_top = px.bar(df_prov_top, x='TOTAL', y='NOMBRE EMISOR', orientation='h', template="plotly_white", color_discrete_sequence=['#2E7D32'], labels={'TOTAL': 'Total Facturado ($)', 'NOMBRE EMISOR': 'Proveedor'})
            fig_prov_top.update_layout(xaxis_tickformat="$ ,.0f", separators=",.")
            fig_prov_top.update_traces(hovertemplate="<b>Proveedor:</b> %{y}<br><b>Total Facturado:</b> $ %{x:,.0f}<extra></extra>")
        ...
        # ==============================================================================
        st.markdown("---")
        st.subheader("🔍 Explorador de Documentos DIAN (Áreas Agendadas - Corte al 19 de Mayo)")
        
        columnas_vista = ["FOLIO", "NOMBRE EMISOR", "TOTAL", "RESPONSABLE", "FECHA EMISIÓN"]
        df_tabla = df_filtrado[columnas_vista].sort_values(by="FECHA EMISIÓN", ascending=False)
        
        st.dataframe(
            df_tabla.style.format({
                'TOTAL': lambda x: f"$ {x:,.0f}".replace(',', 'X').replace('.', ',').replace('X', '.'),
                'FECHA EMISIÓN': lambda t: t.strftime('%d/%m/%Y') if pd.notnull(t) else ""
            }), 
    ...
    st.error(f"❌ No se encontró el archivo de Excel en la ruta especificada: {EXCEL_PROV}")
